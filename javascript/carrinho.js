...
// id_vendedor do localStorage; situacao:"aprovado"; numero_pedido_ecommerce = id_vendedor-YYYYMMDD-HHmmss.
...
export function init(root) {
  ...
  function getVendedorFromLS(){
    try{
      ...
      if(keys.length){ const cpfKey=keys[0]; const v=data[cpfKey]||{};
  ...
  function renderCarrinho(){
    ...
    if(!itens.length){ cartEl.innerHTML=`<div class="carrinhoProposta-vazio">Seu carrinho está vazio :(</div>`; recalcPaySummary(); return; }
    ...
    const descontoTotal = itens.reduce((a,it)=>{ const base=(Number(it.preco)||0)*(Number(it.quantidade)||0); const perc=(getDescMode()==='item')?clampPerc(Number(it.descPerc)||0):getDescGlobalPerc(); return a + base*(perc/100); },0);
  ...
  async function hidratarNomes(itensSemNome){
    ...
    await Promise.all(itensSemNome.map(async (item)=>{
      ...
      const sel=(window.CSS&&CSS.escape)? `td.nome-produto[data-sku="${CSS.escape(String(item.sku))}"] .nome-produto-inner` : `td.nome-produto[data-sku="${String(item.sku).replace(/"/g,'\\"')}"] .nome-produto-inner`;
      const span=root.querySelector(sel); if(span){ const texto=nome||"(sem nome)"; span.textContent=texto; span.parentElement.title=texto; }
      const idx=indexBySku.get(String(item.sku)); if(typeof idx==="number" && idx>=0 && carrinhoAtual[idx]){ const norm=s=> (s&&String(s).trim()?String(s).trim():null); carrinhoAtual[idx].nome = norm(nome) ?? carrinhoAtual[idx].nome ?? null; }
    }));
  ...
  root.querySelectorAll('.cep-busca').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      ...
      try{ const data=await buscaCEP(group.cep.value); group.cep.value=onlyDigits(data.cep); group.endereco.value=data.endereco; group.bairro.value=data.bairro; group.cidade.value=data.cidade; group.uf.value=data.uf; }
  ...
  elDoc?.addEventListener('blur', async ()=>{
    ...
    try{
      const res=await fetch(`${PROPOSTA_DB}${CLIENTES_NODE}/${key}.json`); if(!res.ok) throw new Error('erro consulta cadastro');
      const dados=await res.json(); if(dados){
        if(dados.nome)elNome.value=dados.nome; if(dados.tipo)elTipo.value=dados.tipo; if(dados.tel)elTel.value=dados.tel; if(dados.email)elEmail.value=dados.email;
        const c=dados.enderecoCadastro||{}; cad.cep.value=onlyDigits(c.cep||""); cad.endereco.value=c.endereco||""; cad.bairro.value=c.bairro||""; cad.cidade.value=c.cidade||""; cad.uf.value=c.uf||""; cad.numero.value=c.numero||""; cad.complemento.value=c.complemento||"";
        const e=dados.enderecoEntrega||null; chkEnt.checked=!!e; entBox.style.display=chkEnt.checked?'block':'none';
        if(e){ ent.cep.value=onlyDigits(e.cep||""); ent.endereco.value=e.endereco||""; ent.bairro.value=e.bairro||""; ent.cidade.value=e.cidade||""; ent.uf.value=e.uf||""; ent.numero.value=e.numero||""; ent.complemento.value=e.complemento||""; }
  ...
  function feedbackBotoes(){ const b1=root.querySelector('#carrinhoProposta-btn-proposta'); const b2=root.querySelector('#carrinhoProposta-btn-pedido'); if(b1) b1.disabled=!validarProposta(); if(b2) b2.disabled=!validarPedido() || calcularSaldo()>0; }
  function showMsg(texto, erro=false){ const m=root.querySelector('#carrinhoProposta-msg'); if(!m) return; m.innerHTML=texto; m.className='carrinhoProposta-msg'+(erro?' carrinhoProposta-erro':''); m.style.display='flex'; setTimeout(()=>{ m.style.display='none'; }, 3000); }
  ['input','change','blur'].forEach(ev=>{ elNome?.addEventListener(ev,feedbackBotoes); elDoc?.addEventListener(ev,feedbackBotoes); }); feedbackBotoes();

  // ===== PROPOSTA (RTDB) =====
  function coletarClienteRTDB(){
    const entregaDif=chkEnt?.checked;
    const cadastro={ cep:onlyDigits(cad.cep.value), cidade:cad.cidade.value.trim(), uf:cad.uf.value.trim(), endereco:cad.endereco.value.trim(), bairro:cad.bairro.value.trim(), numero:cad.numero.value.trim(), complemento:cad.complemento.value.trim() };
    const entrega=entregaDif? { cep:onlyDigits(ent.cep.value), cidade:ent.cidade.value.trim(), uf:ent.uf.value.trim(), endereco:ent.endereco.value.trim(), bairro:ent.bairro.value.trim(), numero:ent.numero.value.trim(), complemento:ent.complemento.value.trim() } : null;
    return { nome:elNome.value.trim(), tipo:elTipo.value, cpfCnpj:cpfCnpjKey(elDoc.value), tel:elTel.value.trim(), email:elEmail.value.trim(), entregaDiferente:entregaDif, enderecoCadastro:cadastro, enderecoEntrega:entrega };
  }
  function calcularTotais(){
    const itens=getCarrinho();
    const subtotalBruto=itens.reduce((a,it)=>a+(Number(it.preco)||0)*(Number(it.quantidade)||0),0);
    const descTotal=itens.reduce((a,it)=>{ const base=(Number(it.preco)||0)*(Number(it.quantidade)||0); const perc=(getDescMode()==='item')?clampPerc(Number(it.descPerc)||0):getDescGlobalPerc(); return a + base*(perc/100); },0);
    const total=subtotalBruto-descTotal; return { mode:getDescMode(), descGlobal:getDescGlobalPerc(), subtotalBruto, descontoTotal:descTotal, total };
  }

  async function upsertCliente(cliente){
    const key=cpfCnpjKey(cliente.cpfCnpj); if(!key) throw new Error("CPF/CNPJ ausente");
    const res=await fetch(`${PROPOSTA_DB}${CLIENTES_NODE}/${key}.json`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ ...cliente, updatedAt:nowISO() }) });
    if(!res.ok) throw new Error("Falha ao salvar cadastro"); return key;
  }
  ...
  async function salvarProposta(status = "rascunho") {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Falha ao salvar a proposta");
  ...
  payAddBtn.addEventListener('click', ()=>{
    ...
    if(tipo==='credito'){ base.gateway=(payGW.value||'').trim(); base.parcelas=parseInt(payInst.value||'1',10)||1; }
  ...
  function renderPayments(){
    ...
    payments.forEach(p=>{
      ...
      card.innerHTML=`
        ...
        <div class="gw-wrap" ${p.tipo==='credito'?'':'style="display:none"'}>
          <label>Gateway / Meio</label>
          <input class="gw" value="${p.gateway||''}">
        </div>
        <div class="inst-wrap" ${p.tipo==='credito'?'':'style="display:none"'}>
  ...
  function itensParaTiny(){
    ...
    return itens.map(it=>{
      ...
      return { codigo:String(it.sku), quantidade:String(qtd), valor_unitario:String(unit.toFixed(2)) };
    });
  }
    ...
    return payments
      ...
      .map(p=>{
        const base={
          ...
          obs:(p.tipo==='credito' && p.parcelas>1)?`Cartão crédito ${p.parcelas}x`:''
        };
        if(p.tipo==='pix'){
          base.meio_pagamento = PIX_MEIO_FIXO; // sempre Santander
  ...
  // ====== SALVAR PEDIDO NO RTDB (/pedidos/<vendedorKey>/<numero_pedido_ecommerce>) ======
  async function salvarPedidoRTDB({ vendedorKey, numero_pedido_ecommerce, clienteRTDB, itensSnapshot, totais, payloadEnviado, tinyResposta }) {
    const registro = {
      createdAt: nowISO(),
      ...
      itens: itensSnapshot,
      totais,
      payloadTiny: payloadEnviado,   // o que foi enviado ao proxy/Tiny
      tinyRetorno: tinyResposta,     // retorno da API Tiny
      vendedor: {
        id: VENDEDOR?.id ?? null,
  ...
  // ===== Boot =====
  renderControls(); renderCarrinho();

  // ===== Teardown =====
  return function teardown(){ window.removeEventListener("storage", onStorage); window.removeEventListener("resize", onResize); };
}
