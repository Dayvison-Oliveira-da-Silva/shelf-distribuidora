...
// calcula vendorKey a partir de um registro (para legado)
...
// caminho direto por ID
function buildNestedItemUrl(vendedorKey, id) {
  return `${PROPOSTAS_DB}/propostas/${encodeURIComponent(vendedorKey)}/${encodeURIComponent(id)}.json`;
}
  ...
  modal.innerHTML = `
    <div id="pp-modal-card" style="background:#fff; width:min(980px, 96vw); max-height:90vh; overflow:auto; border-radius:14px; box-shadow:0 10px 30px rgba(0,0,0,.25);">
      ...
      <div style="padding:14px 16px;">
        ...
        <button id="pp-btn-additem" class="pp-btn-sec" style="margin:8px 0;">+ Adicionar item</button>
...
function openModalWith(proposta, vendorKey, id) {
  ...
  // hidrata estado (preservando snapshot: nome, marca, imagemUrl)
  ...
  Modal.state = {
    ...
    cliente: {
      ...
      cpfCnpj: c?.cpfCnpj || "",
      tel: c?.tel || "",
      enderecoCadastro: c?.enderecoCadastro || {},
      enderecoEntrega: c?.enderecoEntrega || null,
...
function computeTotals() {
  ...
  let desconto = 0;
  if (Modal.state.descMode === "global") {
    desconto = subtotal * ((Number(Modal.state.descGlobal)||0) / 100);
  } else {
    desconto = itens.reduce((acc, it)=>{
      const base = (Number(it.preco)||0)*(Number(it.quantidade)||0);
      const perc = Number(it.descPerc)||0;
      return acc + base*(perc/100);
    }, 0);
  }
  const total = subtotal - desconto;
  Modal.state.totais = {
    mode: Modal.state.descMode,
    descGlobal: Number(Modal.state.descGlobal)||0,
...
function renderModal() {
  ...
  stBadge.setAttribute("style", badge(s.status));
  ...
  el.querySelectorAll('input[name="pp-desc-mode"]').forEach(r => {
    r.checked = (r.value === s.descMode);
  });
  const gInp = el.querySelector("#pp-f-desc-global");
  gInp.value = String(s.descGlobal).replace(".", ",");
  gInp.disabled = (s.descMode !== "global");

  // itens
  const tbody = el.querySelector("#pp-items tbody");
  tbody.innerHTML = "";
...
function wireModalHandlers() {
  ...
  el.querySelector("#pp-items").addEventListener("input", (ev)=>{
    ...
    const it = Modal.state.itens[idx];
    if (!it) return;
  ...
  el.querySelector("#pp-items").addEventListener("click", (ev)=>{
    ...
    renderModal(); // re-render para reindexar inputs
  });
...
function updateRowSubtotal(idx) {
  ...
  const it = Modal.state.itens[idx];
  if (!el || !it) return;
  const mode = Modal.state.descMode;
  const perc = (mode === "item") ? (Number(it.descPerc)||0) : 0;
...
async function saveModalChanges() {
  try {
    ...
    // payload (PRESERVA snapshot dos itens)
    computeTotals();
    const payload = {
      ...
      clienteSnapshot: {
        ...Modal.state.cliente,
        cpfCnpj: onlyDigits(Modal.state.cliente.cpfCnpj || "")
      },
      ...
      totais: {
        mode: Modal.state.descMode,
        descGlobal: Number(Modal.state.descGlobal)||0,
        subtotalBruto: Modal.state.totais.subtotalBruto,
        descontoTotal: Modal.state.totais.descontoTotal,
        total: Modal.state.totais.total
      },
      vendedor: { // preserva vendedor
        id: VENDEDOR?.id ?? null,
...
async function cloneCurrentProposal() {
  try {
    const clone = structuredClone({
      ...
      updatedAt: nowISO(),
      status: "rascunho",
      clienteSnapshot: Modal.state.cliente,
      itens: Modal.state.itens, // já contém nome/marca/imagemUrl
      totais: Modal.state.totais,
      vendedor: {
        id: VENDEDOR?.id ?? null,
        nome: VENDEDOR?.nome ?? null,
        tipo: VENDEDOR?.tipo ?? null,
    ...
    // recompute totais com estado atual
    computeTotals();
    clone.totais = { ...Modal.state.totais };

    const url = buildNestedUrl(Modal.state.vendorKey);
    const { name: newId } = await postJSON(url, clone);

    // insere na lista e abre o novo
    ...
    alert("Falha ao clonar proposta.");
  }
}
...
async function shareCurrentProposal() {
  ...
  try {
    if (navigator.share) {
      await navigator.share({ title: "Proposta", url: link });
    } else {
...
function buildOpenLink(vendorKey, id) {
  const u = new URL(window.location.href);
  u.searchParams.set("open", id);
  u.searchParams.set("vendor", vendorKey);
  return u.toString();
}

// imprimir / pdf (abre uma janela de impressão)
...
function mountRow(p, vendorKeyForRow) {
  ...
  btn.className = "pp-btn-sec";
  btn.addEventListener("click", () => {
    const vendorKey = vendorKeyForRow || vendorKeyFromRecord(p) || buildVendorKey(VENDEDOR);
    openModalWith(p, vendorKey, p.key);
  });
...
// -------------------- Módulo principal --------------------
let BASE_DATA = []; // [{key, ...proposta}]
let CURRENT_VENDOR_KEY = null;
let UI = {};

export function init(rootMainEl) {
  // pega elementos inseridos pelo propostas.html
  UI = {
...
async function loadAll() {
  try {
    ...
    if (UI.erroEl) {
      UI.erroEl.textContent = e?.message || "Falha ao carregar propostas.";
      UI.erroEl.style.display = "block";
    }
...
function renderList() {
  ...
  if (!view.length) {
    UI.vazioEl && (UI.vazioEl.style.display = "block");
    return;
  }
  UI.vazioEl && (UI.vazioEl.style.display = "none");
